...
void wrq_send(char *,char *,int);
void rrq_send(char *, char *,int);
void initial(char *,struct sockaddr_in*,int*,int*,int*,char *);
...
void upload(struct hostent *hostname ,char *filename)
{
    ...
    */
    ...
    while(1)
    {
    	if(bno%21==0)
    		flag = 1;
    	initial(data,&r_addr,&alen,&ack,&bno,msg);
...
void wrq_send(char *msg, char *filename,int len)
{
	bzero(msg, 516);
	...
	strcpy(msg+2+len+1, "octet"); //mode field contains the string "netascii", "octet", or "mail" 
	msg[2+len+strlen("octet")+1] = 0x0; // 0 byte
	int req_len=2+len+1+strlen("octet")+1; 
...
void rrq_send(char *msg,char *filename,int len)
{
	bzero(msg, 516);
...
void initial(char *data,struct sockaddr_in *r_addr,int *alen,int *ack,int *bno,char *msg)
{
	int i;
	for(i=0;i<5;i++)
    	{
